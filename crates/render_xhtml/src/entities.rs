//! Port of `XhtmlParser.DEFINED_ENTITIES`, populated by `initEntities`
//! (XhtmlParser.java:1398-3523). This is the HTML5 named-entity table.
//!
//! Extracted verbatim from the Java source (keys and values byte-for-byte),
//! INCLUDING fhir-core's own quirks that we reproduce for fidelity:
//!   * `&NotGreaterFullEqual;` -> U+2267 U+0033 (Java literal `≧3`;
//!     the second scalar is `0033` not `0338` in the source — reproduced as-is).
//!   * `&NotGreaterGreater;`   -> U+226B U+0033 (same class of source quirk).
//!   * `&bsol;` -> the 6-char string `\` (Java literal `"\\u005C"`, i.e. an
//!     escaped backslash followed by `u005C`, NOT a unicode escape).
//!
//! Keys include the leading `&` and trailing `;`, matching how the parser looks
//! them up: `DEFINED_ENTITIES.containsKey("&"+c+";")` (XhtmlParser.java:1033).

/// Look up a full entity token (e.g. `"&amp;"`) -> replacement string.
pub fn defined_entity(token: &str) -> Option<&'static str> {
...
pub static ENTITIES: &[(&str, &str)] = &[
    ...
    ("&NotGreaterFullEqual;", "\u{2267}\u{33}"),
    ("&NotGreaterGreater;", "\u{226B}\u{33}"),
    ...
    ("&bsim;", "\u{223D}"),
    ("&bsime;", "\u{22CD}"),
    ("&bsol;", "\u{5C}\u{75}\u{30}\u{30}\u{35}\u{43}"),
    ("&bsolb;", "\u{29C5}"),
    ("&bsolhsub;", "\u{27C8}"),
    ("&bull;", "\u{2022}"),
    ("&bullet;", "\u{2022}"),
