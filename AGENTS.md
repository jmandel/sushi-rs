...
**PROPER INCREMENTALISM, SNAPSHOT SLICE (ACTIVE, UNCOMMITTED 2026-07-15):**
SiteEngine privately reuses individual StructureDefinition snapshot results
only after revalidating complete opaque PackageContext dependency manifests
against the current package/local-resource context. Manifests cover positive
and negative fetches, selected body/order, local/package provenance, package
id, canonical version, and recursive reads. Current authored inputs still
produce a new complete PreparedGuide; canonical compilation and the public four
operations are unchanged. Staging and promotion are atomic, retained history is
current+previous successful semantic generations, and empty/over-budget
successes insert zero-data tombstones so older facts cannot outlive that bound.
Values/manifests are `Rc`-shared; reported manifest bytes are deterministic
approximate admission weight and generated bytes are serialized-JSON weight.

Release snapshot_gen, SiteEngine, and Fig focused suites are green (15 unit
plus all snapshot integrations; 36/1; 14/1). The authoritative frozen receipt
at `target/incremental-differential/snapshot-reuse-four-guide-final2-20260715/aggregate.json`
passes 4/4: Tiny/IPS/US Core/mCODE compare 1,799/2,233/3,848/2,872 complete
outputs in both render orders across compilation, diagnostics, closed-build
closure/bytes, catalogs, every ContentRef/body, and final SiteOutput. Partial
resource reuse is deliberate for Tiny/IPS/US Core; mCODE's site-only mutation
deliberately whole-hits. Tiny also proves typed failed-successor nonpromotion
and fully rendered/finalized same-engine recovery. Native retained-vs-fresh-B
PreparedGuide time is 37/41 ms Tiny, 466/660 ms IPS, and 236/573 ms US Core;
retained full Rust prepare is 144/318 ms, 783/1,154 ms, and 1,007/1,631 ms.
Canonical render-context work is now the main measured semantic-edit residual
(about 230 ms IPS and 577 ms US Core). WASM/browser measurement, safe render-
context reuse, docs, and landing remain pending. Do not push unfinished work
merely because the user allowed pushes from the new parent checkout. Do not
reopen R4/R5.

**RENDER PACKAGE-CATALOG REUSE (CERTIFIED LOCALLY, UNCOMMITTED 2026-07-15):**
IgContext's package metadata is an immutable `Rc` catalog, while own resources,
tx externals, tree, and mixed lazy lookup/body caches remain fresh. The outer
key binds primary IG identity, first FHIR version, ordered dependsOn, exact
compile-lock carriers, resolved-label order, root, recipe/API; render_sd
separately checks ordered dependsOn and selected core. Only package entries
cross a semantic edit. Candidates promote after complete runtime installation
and current+previous is the hard bound. Metrics split catalog hit/build/
entries/generations, fresh own-context build, render-semantics time, and
RenderState time. Focused selection/core, fresh-own-shadow, carrier/root,
semantic, identity-miss, and bounded A -> B -> A tests pass. The authoritative
receipt is
`target/incremental-differential/render-package-catalog-four-guide-v2-20260715/aggregate.json`:
`pass`, 4/4, 1,799/2,233/3,848/2,872 outputs. Retained/fresh render-semantics
time is 79/98 ms Tiny, 154/245 ms IPS, and 343/605 ms US Core; RenderState is
only .15/2.5/30 ms.

**PREPARSED OWN-RESOURCE + BOUNDED CATALOG FOLLOW-UP (ACTIVE, UNCOMMITTED
2026-07-15):** render_surface passes current PreparedGuide values directly into
IgContext, deleting the own-resource JSON tree round trip while rebuilding all
current own/tx/mixed caches. Independent review found pseudo-JSON's rare
referenced-own-SD branch still read only the tree. It now follows one shared
`load_resource_path` seam, and a profiled-type regression requires identical
filesystem/preparsed bytes and the actual SD type anchor. Catalog retention is
bounded at 1,024 packages, 100,000 resource/spec entries, 32 MiB deterministic
logical weight, and current+previous generations. Over-budget successes install
same-key tombstones. Real Publisher preparation tests prove a post-staging
failure cannot promote, an over-budget tombstone forces the next rebuild, and
normal admission recovers. Focused tests pass. The pre-fix bounded differential
was interrupted and is non-evidence. The corrected authoritative frozen receipt
`target/incremental-differential/render-bounded-own-four-guide-final2-20260715/aggregate.json`
passes all 4 guides and 1,799/2,233/3,848/2,872 complete outputs. Retained/fresh
Rust prepare is 128/313 ms Tiny, 680/1,134 ms IPS, 722/1,604 ms US Core, and
122/1,242 ms mCODE; retained/fresh render semantics is 71/88, 111/198, and
266/532 ms for Tiny/IPS/US Core. Catalog logical weights are .25/2.72/7.22/4.19
MB, all below the 32 MiB bound.

**INCREMENTAL ROUND WASM/BROWSER RESULT (2026-07-15):** current-source native
suites, workspace check/fmt, Rust 1.96 WASM 15+5+8, app 149/149, Pages build,
and `/tmp/fhir-incremental-render-full-browser-final.log` all pass. The complete
browser gate covers Tiny/Cycle, US Core 1,535/1,535 images and 85/85 assets with
one shell, real mCODE, restart, scroll, mobile, and registry retry. The exact
one-repeat fast aggregate is
`target/benchmark-results/incremental-render-fast-once-reviewed-final-20260715/aggregate.json`
and `ok:true`: edit times are 1.225/2.704/3.554/1.550 s for Tiny/IPS/US Core/
mCODE, with zero open network boundaries. US Core compilation is only 105 ms
and IPS 572 ms, so unsafe declaration-level reuse is not pursued. The editor's
shared identity module now globally byte-sorts, rejects non-regular entries,
propagates helper failure, and proves the child-served frozen tree matches the
matrix source artifact; the failed earlier aggregates are non-evidence. The 41
identity/lifecycle tests and independent final review are clean. Committed-stamp
rebuild, landing, Pages, and live verification remain.
