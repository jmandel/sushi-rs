...
# committed.
BUILD_ROOT="${BUILD_ROOT:-$REPO_ROOT/../sushi-rs-snapshot-f0-builds}"
...
#    outliers over MAX_FRAGMENT_BYTES (logged to excluded-fragments.txt).
rm -rf "$OUT"
...
echo "fragments: total=$frag_total kept=$frag_kept excluded_en=$frag_en excluded_big=$frag_big"

# 4. Harvest page HTML if Jekyll produced final output.
JEKYLL_OK=no
page_count=0
if [[ -d "$BUILD/output" ]]; then
  # Count real page HTML in output (jekyll-rendered). If present, harvest.
  page_count=$(find "$BUILD/output" -maxdepth 1 -name '*.html' | wc -l)
  if (( page_count > 0 )); then
    JEKYLL_OK=yes
    mkdir -p "$OUT/pages"
    find "$BUILD/output" -maxdepth 1 -name '*.html' -exec cp {} "$OUT/pages/" \;
  fi
fi
echo "pages: jekyll_ok=$JEKYLL_OK count=$page_count"
...
- **Isolated HOME**: yes (never touched real ~/.fhir)
- **Harvest date**: $(date -u +%Y-%m-%dT%H:%M:%SZ)
- **Command**: \`HOME=<isolated> java -Xmx6g -jar publisher.jar -ig <build-dir>\`
- **Publisher exit code**: $PUB_RC
- **Publisher wall time**: ${RUN_SECS}s

## Corpus
- Fragments (base *.xhtml, -en dupes excluded): **$frag_kept** kept ($frag_du)
  - total emitted (incl -en): $frag_total; -en excluded: $frag_en; >${MAX_FRAGMENT_BYTES}B excluded: $frag_big
- Pages (output/*.html): **$page_count** ($page_du), jekyll_ok=$JEKYLL_OK
