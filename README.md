...
Current Publisher Liquid behavior and its executable gate are documented in
[`crates/render_liquid/README.md`](crates/render_liquid/README.md). Publisher
SQL is a separate pre-Liquid close step: `publisher_sql` builds one bounded,
read-only in-memory SQLite snapshot from the guide's own compiled resources,
rewrites SQL sources once into ordinary includes and global `_data`, and then
disappears from the render lifecycle. This is an explicitly partial,
Rust-characterized `package.db` surface; dependency/terminology tables and full
Java compatibility await a pinned oracle. Cycle deliberately has neither that
database nor a SQL tag. Historical corpus counts below remain regression
evidence, not a current support matrix.
...
## Historical parity corpus (the regression floor)
...
Those figures are pinned regression-oracle results from the focused snapshot,
...
They are not regenerated by the ordinary workspace test suite; a new current
claim requires explicit captured oracle inputs and a retained receipt.
...
That same closed carrier is also the immutable proof used when a later
preparation revalidates a package-backed snapshot dependency. The manifest
stores an opaque, domain-separated identity of carrier `ContentRef` plus member
path; an exact match avoids a redundant member inflate/parse, while disk,
mutable, changed, or otherwise unproven sources use the canonical read/parse
comparison. This refines one dependency fact inside preparation and does not
add another cache or lifecycle operation.
...
Both targets use the same transaction: one owned compilation candidate and one
owned target candidate remain off-side until every fallible target-close and
object-admission check succeeds. One infallible success commit advances exactly
three bounded current/previous histories—semantic compilation, prepared
derivations, and installed runtimes. A failed Publisher or Cycle candidate
